...
export async function uploadToStorage({ id, buffer, filename }: UploadParams): Promise<string> {
...
export async function deleteFromStorage({ id, filename }: DeleteParams): Promise<boolean> {
  try {
    const uploadDir = join(process.cwd(), "uploads", id.slice(0, 2));
    const filePath = join(uploadDir, `${id}-${filename}`);
