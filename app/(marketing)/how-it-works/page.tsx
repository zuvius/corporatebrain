        ...
        <section className="relative overflow-hidden py-20 lg:py-28">
          ...
          <div className="relative mx-auto max-w-4xl px-6 text-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 mb-6">
              ...
              <span className="text-sm font-medium text-violet-300">3 Simple Steps</span>
            </div>
            ...
            <p className="text-lg text-white/50 max-w-2xl mx-auto">
              From scattered documents to intelligent answers in three simple steps.
            </p>
          </div>
        </section>
...
function Step({ number, title, description, points, icon: Icon, reversed = false }: { number: string; title: string; description: string; points: string[]; icon: React.ElementType; reversed?: boolean }) {
  return (
    <div className={`flex flex-col ${reversed ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-12 lg:gap-20 items-center`}>
      ...
      <div className="flex-1">
        <div className="text-violet-400 font-bold text-lg mb-2">STEP {number}</div>
...
function TechCard({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) {
  return (
...
function UseCase({ icon: Icon, title, example }: { icon: React.ElementType; title: string; example: string }) {
  return (
    <div className="group relative rounded-2xl border border-white/10 bg-white/[0.02] p-6 hover:border-violet-500/30 hover:bg-white/[0.04] transition-all duration-300">
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-violet-600/5 to-fuchsia-600/5 opacity-0 group-hover:opacity-100 transition-opacity" />
      <div className="relative">
...
function SecurityBadge({ icon: Icon, text }: { icon: React.ElementType; text: string }) {
  return (
